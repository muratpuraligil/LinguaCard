...
const CustomSetManager: React.FC<CustomSetManagerProps> = ({ words, onExit, onPlaySet, onUploadNewSet, onRefresh, onRenameCustomSetLocally }) => {
    ...
    const allSets = useMemo(() => {
        ...
        return Object.entries(grouped).map(([name, setWords]) => {
            const trEnKey = `lingua_set_progress_${name.replace(/\s+/g, '_')}_TR_EN`;
            const enTrKey = `lingua_set_progress_${name.replace(/\s+/g, '_')}_EN_TR`;
            
            const trEnSaved = localStorage.getItem(trEnKey);
            const enTrSaved = localStorage.getItem(enTrKey);
            
            let trEnCount = 0;
            let enTrCount = 0;

            if (trEnSaved) {
                const data = JSON.parse(trEnSaved);
                trEnCount = Object.values(data).filter((p: any) => p?.status === 'CORRECT').length;
            }
            if (enTrSaved) {
                const data = JSON.parse(enTrSaved);
                enTrCount = Object.values(data).filter((p: any) => p?.status === 'CORRECT').length;
            }

            // Eski tekil key desteği (Migration)
            if (trEnCount === 0 && enTrCount === 0) {
                const oldSaved = localStorage.getItem(`lingua_set_progress_${name.replace(/\s+/g, '_')}`);
                if (oldSaved) {
            ...
            const completed = Math.max(trEnCount, enTrCount);
    ...
    const SetCard: React.FC<{ set: SetInfo }> = ({ set }) => {
        const isSampleSet = set.name === 'am-is-are' || set.name === 'Demo Kelimeler';
        ...
        let statusConfig = {
            ...
            label: "Başlanmadı",
            btnIcon: <Play size={16} fill="currentColor" />,
        ...
        return (
            <div className={`bg-zinc-900 border border-white/5 rounded-[24px] p-5 flex flex-col justify-between group transition-all relative overflow-hidden h-[260px] hover:bg-zinc-800 hover:border-white/10 shadow-xl ${isSampleSet ? 'ring-2 ring-yellow-500/20' : ''}`}>
                ...
                <div className="flex justify-between items-start mb-3">
                    <div className={`flex items-center gap-2 px-2 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${statusConfig.bgColor} ${statusConfig.color} border ${statusConfig.borderColor}`}>
                        {statusConfig.icon}
                    ...
                    {!isSampleSet && (
                        <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button
                                ...
                                className="p-1.5 bg-white/5 text-slate-400 hover:text-blue-400 rounded-lg transition-all"
                            >
                                <Pencil size={14} />
                            </button>
                            <button
                                onClick={(e) => { e.stopPropagation(); setSetToDelete(set.name); }}
                                className="p-1.5 bg-white/5 text-slate-400 hover:text-red-400 rounded-lg transition-all"
                            >
                                <Trash2 size={14} />
                            </button>
                        </div>
                ...
                <div className="mb-2 flex-1">
                    <h3 className="text-lg font-black text-white mb-1 leading-tight group-hover:text-blue-400 transition-colors tracking-tight line-clamp-2">
                        {set.name}
                    </h3>
                ...
                <div className="mt-auto">
                    <div className="flex justify-between items-end mb-2">
                        <span className="text-xl font-black text-white tracking-tighter">{set.completed}<span className="text-xs text-slate-600 font-bold">/{set.total}</span></span>
        ...
        <div className="min-h-screen bg-black text-white font-['Plus_Jakarta_Sans'] p-6 md:p-10 flex flex-col items-center">
            <div className="w-full max-w-5xl">
                <div className="flex items-center gap-4 mb-8">
                    <button onClick={onExit} className="p-3 bg-zinc-900 rounded-xl hover:bg-zinc-800 transition-all border border-white/5 group shadow-lg">
                ...
                <div className="mb-16">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {ongoingSets.length > 0 ? (
                            ...
                            <div className="col-span-full py-12 text-center text-slate-500 font-bold bg-zinc-900/30 rounded-3xl border border-dashed border-white/10">
                                Henüz devam eden bir çalışmanız yok. Yukarıdan yeni bir set oluşturun.
                            </div>
                        )}
                    </div>
                </div>
